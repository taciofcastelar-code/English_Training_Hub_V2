const CACHE='sentence-building-v2-intelligence-1';
const ASSETS=['./','./index.html','./manifest.webmanifest','../icon.svg'];
self.addEventListener('install',e=>e.waitUntil(caches.open(CACHE).then(c=>c.addAll(ASSETS)).then(()=>self.skipWaiting())));
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!==CACHE && k.startsWith('sentence-building')).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
